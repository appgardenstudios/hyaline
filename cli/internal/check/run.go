...
func Run(check config.Check, system string, current *sql.DB) (result *rule.Result, err error) {
	switch check.Rule {
	case rule.SectionExistsRule:
		...
		result, err = rule.RunSectionExists(check.ID, check.Description, options, system, current)
	default:
