...
func TestCheckCurrentCustomerDocs(t *testing.T) {
	scenarioName := "check-current-customer-docs"
	...
	runFunc := func(iteration int) (*CheckResult, error) {
		...
		args := []string{
			"check", "current",
			"--config", "./_input/check-current-customer-docs/config.yml",
			"--system", "url-shortener",
			"--current", "./_input/check-current-customer-docs/current.sqlite",
			"--output", outputPath,
			"--check-purpose",
			"--check-completeness",
		}
