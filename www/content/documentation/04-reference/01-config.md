...
**branch**: The branch to extract source code from. If not set will default to `main`. Tries to resolve to a local branch first, then a remote branch (if there is a single remote), and finally a tag.
...
**branch**: The branch to extract documentation from. If not set will default to `main`. Uses alias resolution: tries `refs/heads/<branch>`, then `refs/remotes/<remote>/<branch>` (if only one remote exists), then `refs/tags/<branch>`.
